...
<body>
	...
	<div>
		...
		<form class="form-horizontal" method="post"
			action="${pageContext.request.contextPath}/admin/video/updateVideo.action">
				...
				<div class="col-sm-10">
					<select name="speakerId" class="form-control">
						...
						<c:forEach items="${speaker }" var="speaker">
							<option value="${speaker.id }" ${list.speakerId==speaker.id?"selected":""}>${speaker.speakerName }</option>
						</c:forEach>
					</select>
				</div>
			</div>
				...
				<div class="col-sm-10">
					<select name="courseId" class="form-control">
						...
						<c:forEach items="${course }" var="course">
							<option value="${course.id }" ${list.courseId==course.id?"selected":""}>${course.courseName }</option>
						</c:forEach>
					</select>
				</div>
			</div>
